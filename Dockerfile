...
RUN wget https://www-eu.apache.org/dist/tomcat/tomcat-9/v9.0.35/bin/apache-tomcat-9.0.35.tar.gz -P /tmp
RUN tar -xf /tmp/apache-tomcat-9.0.35.tar.gz -C /opt/tomcat/
RUN chown -R tomcat: /opt/tomcat
RUN ln -s /opt/tomcat/apache-tomcat-9.0.35 /opt/tomcat/latest
